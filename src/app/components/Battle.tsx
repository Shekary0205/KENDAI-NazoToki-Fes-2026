...
export default function Battle() {
  ...
  // バトル画面に入ったらトレーナー現るBGM
  useEffect(() => {
    switchTrack("trainer");
    return () => {
      // 勝利遷移時はBGMを切り替えない（victoryをそのまま引き継ぐ）
      if (!wonRef.current) {
        switchTrack("gym");
      }
    };
